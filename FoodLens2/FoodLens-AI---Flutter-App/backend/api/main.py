...
async def analyze_food(
    ...
    try:
        ...
        try:
            ...
            # Extract text - use barcode reader's OCR data if available, otherwise try Gemini
            extracted_text = ""
            if barcode_data and barcode_data.get("detected_text"):
                # Use the text detected by barcode reader (pytesseract/zbar)
                detected_text = barcode_data.get("detected_text", "")
                print(f"✅ Using barcode reader OCR text: {detected_text}")
                
                # Check if detected text is ONLY numbers (barcode digits)
                # If so, the image quality is poor and we should try Gemini Vision
                text_lines = [line.strip() for line in detected_text.split('\n') if line.strip()]
                has_non_numeric = any(not line.replace('"', '').replace("'", '').replace(' ', '').isdigit() for line in text_lines if len(line) > 2)
                
                if not has_non_numeric and len(text_lines) <= 3:
                    print(f"⚠️ Detected text contains only numbers (barcode), trying Gemini Vision for product text...")
                    gemini_text = ocr_agent.extract_text_from_image(food_image_name)
                    if gemini_text and len(gemini_text) > len(detected_text):
                        print(f"✅ Gemini Vision found more text, using it instead")
                ...
                print("🔍 Extracting text from image using Gemini Vision...")
            ...
            print(f"📋 OCR Result: product_name='{ocr_result.get('product_name', '')}', brand='{ocr_result.get('brand', '')}'")            
            ocr_text = ocr_result.get("full_text", "")
            
            # Process barcode if detected
            barcode_result = None
            if barcode_data and barcode_data.get("barcode"):
                barcode_result = barcode_agent.process_barcode(barcode_data["barcode"])
        finally:
            try:
        ...
        # Step 3: Combine data - use OCR if barcode returns empty product name
        barcode_product_name = barcode_result.get("product_name", "") if barcode_result else ""
        ocr_product_name = ocr_result.get("product_name", "")
        
        # Use barcode data only if it has a valid product name, otherwise use OCR
        if barcode_product_name and barcode_product_name.strip():
            product_name = barcode_product_name
            print(f"✅ Using barcode product name: {product_name}")
        elif ocr_product_name and ocr_product_name.strip():
        ...
        product_type = barcode_agent.classify_product_type(
            product_name,
...
• {alt['name']} ({alt['brand']})
  Score: {alt['health_score']}/100 ({score_diff:+d} points better)
  Benefits: {health_benefits}
  Available at: {alt['where_to_buy']} ({alt['price']})"""
            else:
