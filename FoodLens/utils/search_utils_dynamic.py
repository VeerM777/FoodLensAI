...
class SearchUtils:
    ...
    def search_product_databases(self, product_name: str, barcode: str = None) -> Dict[str, Any]:
        """
        Search OpenFoodFacts and other product databases with strict brand + product matching
        """
        try:
            if barcode:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("products"):
                        # STRICT MATCHING: Prioritize brand name and product type
                        products = data["products"]
                        
                        # Parse search query to extract brand and product type
                        search_parts = product_name.lower().split()
                        
                        # Common generic words to ignore in scoring
                        generic_words = {'the', 'and', 'with', 'for', 'fat', 'medium', 'low', 'high', 
                                       'light', 'lite', 'classic', 'original', 'new', 'pack', 'size'}
                        
                        # Extract important words (brand + product type)
                        important_words = set(word for word in search_parts if len(word) > 2 and word not in generic_words)
                        
                        # First word is usually the brand (e.g., "Delicious")
                        search_brand = search_parts[0] if search_parts else ""
                        ...
                        for product in products[:10]:  # Check top 10 results
                            product_name_result = product.get("product_name", "").lower()
                            brand_result = product.get("brands", "").lower()
                            
                            score = 0
                            max_score = 100
                            
                            # CRITICAL: Brand name MUST match in brands field (not just product name)
                            # Weight: 60% - most important factor
                            if search_brand and brand_result and search_brand in brand_result:
                                score += 60  # Exact brand match in brands field
                                match_type = "brand_match"
                            else:
                                # NO PARTIAL CREDIT - if brand doesn't match, likely wrong product
                                match_type = "no_brand_match"
                                # Skip this product entirely if brand doesn't match
                                continue
                            
                            # Product type/words matching (weight: 40%)
                            result_words = set(word for word in (product_name_result + " " + brand_result).split() 
                                             if len(word) > 2 and word not in generic_words)
                            
                            word_matches = len(important_words & result_words)
                            word_score = (word_matches / len(important_words) * 40) if important_words else 0
                            score += word_score
                            
                            # Calculate final match ratio
                            match_ratio = score / max_score if max_score > 0 else 0
                            
                            details = f"{match_type}, words:{word_matches}/{len(important_words)}"
                        ...
                        # STRICT: Require at least 80% match (brand MUST match + good word overlap)
                        if best_match and best_match_score >= 0.80:
                            barcode = best_match["code"]
                            print(f"🎯 Best match (score: {best_match_score:.2f}, {best_match_details}): {best_match.get('product_name', 'Unknown')}")
                            url = f"https://world.openfoodfacts.org/api/v0/product/{barcode}.json"
                        else:
                            if best_match:
                                print(f"⚠️ No good match found (best: {best_match_score:.2f}, {best_match_details}). Rejecting to avoid false matches.")
                            else:
                                print(f"⚠️ No products found with matching brand '{search_brand}' in brands field.")
