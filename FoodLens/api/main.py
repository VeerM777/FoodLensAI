...
def _create_category_based_nutrition_estimate(product_name: str, brand: str, category: str) -> Dict[str, Any]:
    ...
    Returns typical values for the product category with ingredient estimates
    """
    ...
    category_nutrition = {
        "instant_noodles": {
            ...
            "salt_100g": 2.5,
            "sodium_100g": 1000,
            "ingredients": "Refined Wheat Flour, Palm Oil, Salt, Flavor Enhancers (E621, E635), Preservatives"
        },
        "dairy_beverages": {
            "energy_100g": 60,
            "fat_100g": 3,
        ...
        "carbonated_beverages": {
            ...
            "salt_100g": 0,
            "sodium_100g": 10,
            "ingredients": "Carbonated Water, Sugar, Artificial Flavour, Caffeine, Preservatives (E211)"
        },
        "potato_chips": {
            "energy_100g": 536,
            "fat_100g": 34,
            ...
            "salt_100g": 1.5,
            "sodium_100g": 600,
            "ingredients": "Potatoes, Vegetable Oil (Palm Oil), Salt, Artificial Flavours"
        },
        "biscuits": {
            "energy_100g": 480,
            "fat_100g": 22,
            ...
            "salt_100g": 0.8,
            "sodium_100g": 320,
            "ingredients": "Refined Wheat Flour, Sugar, Palm Oil, Vegetable Fat, Artificial Flavour"
        },
        "chocolates": {
            "energy_100g": 530,
            "fat_100g": 30,
            ...
            "salt_100g": 0.2,
            "sodium_100g": 80,
            "ingredients": "Sugar, Cocoa Butter, Milk Solids, Vegetable Fat, Artificial Flavour"
        },
        "spread": {
            "energy_100g": 680,
            "fat_100g": 75,
            ...
            "salt_100g": 0.02,
            "sodium_100g": 8,
            "ingredients": "Palm Oil, Palmolein, Water, Salt, Emulsifiers (E471, E322), Preservatives (E202), Artificial Flavour, Colour (E160a)"
        },
        "processed_food": {
            "energy_100g": 350,
            "fat_100g": 15,
            ...
            "salt_100g": 1,
            "sodium_100g": 400,
            "ingredients": "Various processed ingredients, preservatives, artificial additives"
        }
    }
    
    # Special handling for fat spreads/butter alternatives
    product_lower = product_name.lower()
    if "spread" in product_lower or ("fat" in product_lower and "vegetable" not in brand.lower()):
        category = "spread"
    
    # Get nutrition for category, default to processed_food
    category_data = category_nutrition.get(category, category_nutrition["processed_food"]).copy()
    ingredients = category_data.pop("ingredients", "Ingredients not available - using category-based estimates")
    
    print(f"✅ Using estimated nutrition for {category}: sugar={category_data['sugars_100g']}g, sodium={category_data['sodium_100g']}mg")
...
async def analyze_food(
    ...
    try:
        ...
        try:
            ...
            food_image_name = os.path.join(temp_dir, f'food_{datetime.now().strftime("%Y%m%d_%H%M%S")}.jpg')
            ...
            barcode_data = None
            if barcode_image:
                barcode_image_name = os.path.join(temp_dir, f'barcode_{datetime.now().strftime("%Y%m%d_%H%M%S")}.jpg')
                try:
            ...
            if barcode_data:
                ...
                # Always use Groq Vision for structured extraction with [BRAND], [PRODUCT] format
                print(f"🔍 Running Groq Vision for structured brand/product extraction...")
                groq_text = ocr_agent.extract_text_from_image(food_image_name)
                if groq_text and len(groq_text) > 50:
                    print(f"✅ Groq Vision returned structured data ({len(groq_text)} chars)")
                    extracted_text = groq_text
                else:
                    # If Groq fails, use EasyOCR text as fallback
                    print(f"⚠️ Groq Vision failed, using EasyOCR fallback")
                    extracted_text = product_text if product_text else detected_text
            else:
        ...
        if not barcode_result or barcode_result.get("status") != 1:
            ...
            if search_product and search_product != "Unknown Product":
                ...
                if name_search_result.get("status") == 1 and name_search_result.get("product"):
                    ...
                    # Use Gemini AI to estimate nutritional data AND ingredients for unknown products
                    gemini_client = search_client.get_gemini_client()
                    if gemini_client:
                        try:
                            # Enhanced prompt to include ingredient estimation
                            nutrition_prompt = f"""Analyze this product and provide nutritional values AND typical ingredients:

Product: {search_product}
Brand: {search_brand}

Based on the product type, provide:

1. NUTRITIONAL VALUES per 100g (realistic estimates for this product category)
2. TYPICAL INGREDIENTS (what products like this normally contain)
...
{{
    ...
    "salt_100g": <grams>,
    "sodium_100g": <grams>,
    "ingredients_text": "<typical ingredients list for this product type>"
}}

IMPORTANT for ingredients_text:
- For "Fat Spread" or "Vegetable Fat Spread": Include palm oil, vegetable oils, emulsifiers, preservatives
- For butter alternatives: Include the actual fats used (palm oil, vegetable fat, etc.)
- For processed foods: Include preservatives, artificial flavors if typical
- Be realistic about what cheap processed products contain

Example for "Vegetable Fat Spread":
"ingredients_text": "Palm Oil, Palmolein, Water, Salt, Emulsifiers (E471, E322), Preservatives (E202), Artificial Flavour, Colour (E160a)"

Provide realistic data based on similar market products."""
                            ...
                            if json_match:
                                ...
                                print(f"✅ AI estimated nutritional data: {ai_data}")
                                
                                # Extract ingredients if provided, otherwise use default based on product type
                                ingredients_text = ai_data.get("ingredients_text", "")
                                if not ingredients_text:
                                    # Fallback ingredient estimation based on product type
                                    if "spread" in search_product.lower() or "fat" in search_product.lower():
                                        ingredients_text = "Palm Oil, Vegetable Fat, Water, Salt, Emulsifiers, Preservatives, Artificial Flavour"
                                    elif "butter" in search_product.lower() and search_brand.lower() != "amul":
                                        ingredients_text = "Palm Oil, Vegetable Oils, Water, Salt, Emulsifiers, Artificial Butter Flavor"
                                    else:
                                ...
                                barcode_result = {
                                    ...
                                    "nutriments": {k: v for k, v in ai_data.items() if k != "ingredients_text"},
                                    "categories": "",
                                    "labels": "AI_ESTIMATED",
        ...
        final_response = {
            ...
            "quality_grade": health_analysis.get("quality_grade", "UNKNOWN"),
            "ingredient_concerns": health_analysis.get("ingredient_concerns", []),
...
Use friendly, conversational tone."""
            ...
            # CASE 2: Regular alternative request (no previous suggestions)
                ...
                # Extract scanned products from context for better recommendations
                last_scanned_product = None
                if context and '📦 RECENTLY SCANNED PRODUCTS' in context:
                    import re
                    # Try to extract the most recent product (first one in the list)
                    product_match = re.search(r'1\.\s+\*\*(.+?)\*\*\s+\((.+?)\)', context)
                    if product_match:
                        last_scanned_product = f"{product_match.group(1)} by {product_match.group(2)}"
                
                scanned_context = ""
                if last_scanned_product:
                    scanned_context = f"\n\n🎯 Context: User recently scanned '{last_scanned_product}'. If they're asking for alternatives without specifying what for, they likely mean alternatives for THIS product."
                
                prompt = f"""As a nutrition expert with access to user's scan history, provide 3-5 specific Indian food alternatives.
...
1. **CHECK SCAN HISTORY**: Look at "📦 RECENTLY SCANNED PRODUCTS" section above
2. **Understand context**: If user asks "give me alternatives" or "what can I eat instead" without specifying what, they mean alternatives to the LAST PRODUCT THEY SCANNED
3. **Reference specific scans**: Start response acknowledging what they scanned (e.g., "Since you scanned [Product Name] which scored [X]/100...")
4. **Address concerns**: If the scanned product had health concerns (palm oil, high sugar, etc.), suggest alternatives that fix those issues
5. **Be specific with brands**: Provide actual Indian brand names available in local markets
...
3. Where to buy (DMart, Reliance Fresh, BigBasket, Amazon India, etc.)
4. Health benefits (especially addressing concerns from scanned product)
5. Why it's better than what they originally scanned

Format your response in a friendly, conversational tone that shows you remember what they scanned."""
...
Keep it practical and easy to follow."""
            ...
            # Parse scanned products from context to provide better responses
            scanned_products_mentioned = []
            if context and '📦 RECENTLY SCANNED PRODUCTS' in context:
                # Extract product names from the enhanced context
                import re
                product_matches = re.findall(r'\*\*(.+?)\*\*', context)
                if product_matches:
                    scanned_products_mentioned = product_matches[:3]  # Get up to 3 products
            
            scanned_context = ""
            if scanned_products_mentioned:
                scanned_context = f"\n\n🔍 Note: User recently scanned these products: {', '.join(scanned_products_mentioned)}"
            
            prompt = f"""As a friendly AI nutrition assistant with full memory of user's scan history, answer this question thoughtfully.
...
1. **CHECK SCAN HISTORY FIRST**: Look at the "📦 RECENTLY SCANNED PRODUCTS" section above
2. **Reference specific products**: If user asks "what did I scan?" or "tell me about the product", mention the EXACT product names, brands, scores, and health concerns from the scan history
3. **Be specific**: Don't say "you scanned a product" - say "You scanned [Product Name] by [Brand] which scored [X]/100"
4. **Use actual data**: Reference the actual health scores, ingredients, and concerns from the scan history
5. **Maintain context**: If they ask follow-up questions about ingredients, nutrition, or alternatives, refer back to the scanned products

Provide:
1. Clear, actionable advice based on their actual scan history
...
5. Suggest follow-up actions if relevant (like "Would you like healthier alternatives to the [Product] you scanned?")

Keep response helpful, specific (use actual product names from scan history!), and encouraging."""
...
async def analyze_by_barcode(
    ...
    """
    Analyze product using only barcode (no image required)
    Perfect for real-time barcode scanning or search results
    """
    try:
        print(f"🔍 Analyzing product by barcode: {barcode} for user: {user_id}")
        ...
        print(f"📦 Looking up barcode in OpenFoodFacts: {barcode}")
        barcode_result = barcode_agent.process_barcode(barcode)
        
        if not barcode_result or barcode_result.get("status") != 1:
            raise HTTPException(
                status_code=404,
                detail="Product not found in database. Please try scanning the product label instead."
            )
        
        # Extract product information from barcode result
        product_name = barcode_result.get("product_name", "Unknown Product")
        brand = barcode_result.get("brand", "")
        
        # Get full product data for nutriments
        full_product_data = barcode_result.get("full_product_data", {})
        nutriments = barcode_result.get("nutriments", full_product_data.get("nutriments", {}))
        ingredients_text = barcode_result.get("ingredients_text", full_product_data.get("ingredients_text", ""))
        
        # Health analysis
        print(f"💊 Analyzing health impact for {product_name}...")
        
        # Prepare product data for health analysis
        product_analysis_data = {
            "product_name": product_name,
            "nutriments": nutriments,
            "ingredients_text": ingredients_text,
            "categories": barcode_result.get("categories", ""),
            "product_type": barcode_result.get("categories", "").split(",")[0] if barcode_result.get("categories") else "processed_food"
        }
        
        health_analysis = health_score_agent.analyze_health_impact(
            product_analysis_data,
            user_profile
        )
        ...
        alternatives_list = []
        if user.tier == "premium":
            print(f"🎯 Finding premium alternatives for {product_name}...")
            
            # Prepare product data for alternatives
            product_for_alternatives = {
                "product_name": product_name,
                "brand": brand,
                "product_type": product_analysis_data.get("product_type", "processed_food"),
                "nutriments": nutriments
            }
            
            alternatives_result = alternatives_agent.find_alternatives(
                product_for_alternatives,
                user_profile
            )
            
            # Extract alternatives list from result
            alternatives_list = alternatives_result.get("alternatives", []) if isinstance(alternatives_result, dict) else []
        
        # Verify claims
        claim_verification_result = None
        if ingredients_text:
            print(f"🔍 Verifying product claims...")
            
            # Prepare product data for claim verification
            product_for_claims = {
                "product_name": product_name,
                "nutriments": nutriments,
                "ingredients_text": ingredients_text
            ...
            claim_verification_result = claim_verification_agent.verify_claims(
                product_for_claims,
                ingredients_text  # OCR text
            )
        
        # Store scan in history
        # Extract verdict string from dict if it's a dict, otherwise use as-is
        verdict_value = health_analysis.get("verdict", "Unknown")
        if isinstance(verdict_value, dict):
            verdict_str = verdict_value.get("title", "Unknown")
        else:
            verdict_str = str(verdict_value)
        ...
        scan_history = ScanHistory(
            ...
            alternatives=json.dumps(alternatives_list) if alternatives_list else "[]",
        ...
        if response_format == "user_friendly":
            ...
            summary = {
                ...
                "brand": brand,
                "health_score": health_analysis.get("health_score", 0),
                "verdict": health_analysis.get('verdict', {}).get('title', 'Unknown') if isinstance(health_analysis.get('verdict'), dict) else str(health_analysis.get('verdict', 'Unknown')),
                "has_alternatives": user.tier == "premium",
                "alternatives_count": len(alternatives_list) if user.tier == "premium" else 0,
                "alternatives": []
            }
            ...
            if user.tier == "premium":
                for alt in alternatives_list:
                    score_diff = alt.get('health_score', 0) - health_analysis.get('health_score', 0)
                    health_benefits = alt.get('health_benefits') or alt.get('key_benefits', 'Healthier alternative')
                    
                    summary["alternatives"].append({
                        "name": alt.get('name', ''),
                        "brand": alt.get('brand', ''),
                        "score": alt.get('health_score', 0),
                        "score_difference": score_diff,
                        "benefits": health_benefits,
                        "availability": alt.get('where_to_buy', ''),
                        "price": alt.get('price', ''),
        ...
        print(f"✅ Barcode analysis complete for {product_name}")
...
async def chat_with_ai(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 🆕 Load recent scan history (last 3 scanned products with full details)
        recent_scans = db.query(ScanHistory).filter(
            ScanHistory.user_id == request.user_id
        ).order_by(ScanHistory.timestamp.desc()).limit(3).all()
        
        print(f"📊 Loading chatbot context for user {request.user_id}:")
        print(f"   - Chat messages: {len(recent_chats)}")
        print(f"   - Scanned products: {len(recent_scans)}")
        
        # Build conversation context from history
        conversation_history = []
        suggested_items_history = set()
        scanned_products_details = []
        
        # 🆕 Extract detailed scan history (most important context!)
        for scan in recent_scans:
            try:
                analysis_data = json.loads(scan.analysis_data) if scan.analysis_data else {}
                
                # Extract key information about the scanned product
                product_info = {
                    ...
                    "concerns": analysis_data.get("ingredient_concerns", [])
                }
                
                scanned_products_details.append(product_info)
                print(f"   ✅ Loaded scan: {product_info['name']} (Score: {product_info['score']})")
                
            except Exception as e:
                print(f"   ⚠️ Failed to parse scan history: {e}")
                continue
        ...
        # Build enhanced context with conversation memory AND scan history
        enhanced_context = ""
        
        # 🆕 MOST IMPORTANT: Include detailed scan history
        if scanned_products_details:
            enhanced_context += "📦 RECENTLY SCANNED PRODUCTS (with full details):\n"
            for i, product in enumerate(scanned_products_details, 1):
                enhanced_context += f"\n{i}. **{product['name']}** ({product['brand']})\n"
                enhanced_context += f"   - Health Score: {product['score']}/100\n"
                enhanced_context += f"   - Verdict: {product['verdict']}\n"
                enhanced_context += f"   - Category: {product['category']}\n"
                if product['ingredients']:
                    enhanced_context += f"   - Ingredients: {product['ingredients'][:200]}...\n"
                if product['concerns']:
                    enhanced_context += f"   - Health Concerns: {', '.join(product['concerns'][:3])}\n"
                enhanced_context += f"   - Scanned: {product['timestamp']}\n"
            enhanced_context += "\n"
        ...
        if request.context:
            enhanced_context += f"🔍 CURRENT CONTEXT: {request.context}\n"
        
        print(f"📝 Built enhanced context ({len(enhanced_context)} chars) with {len(scanned_products_details)} scanned products")
...
news_cache = {}
# Search results cache with timestamp
search_cache = {}
...
async def search_products(
    ...
    Returns list of products with basic info for autocomplete
    """
    try:
        print(f"🔍 Searching products for query: '{query}'")
        ...
        if cache_key in search_cache:
            ...
            if time_diff < SEARCH_CACHE_TTL:
                print(f"✅ Using cached results for '{query}' (age: {int(time_diff)}s)")
                return cached_results
        
        # Get or create user
        user = get_or_create_user(user_id, db)
        
        # Fast OpenFoodFacts search with optimized parameters
        products = []
        ...
        try:
            ...
            params = {
                ...
                "json": 1,
                "page_size": limit,
                "fields": "product_name,brands,code,image_url,image_front_url,categories,nutriments"
            }
            
            print(f"🌐 Calling OpenFoodFacts API: {search_url} with query: {query}")
            response = requests.get(search_url, params=params, timeout=5)  # Increased to 5s timeout
            print(f"📊 Response status: {response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                print(f"📦 API returned {data.get('count', 0)} total products")
                ...
                if data.get("products"):
                    ...
                    for product_data in data.get("products", [])[:limit]:
                        ...
                        sodium = (nutriments.get("sodium_100g", 0) or 0) * 1000  # Convert to mg
                        fiber = nutriments.get("fiber_100g", 0) or 0
                        
                        # Simple health score calculation
                        health_score = 100
                        if sugar > 15: health_score -= 20
                        elif sugar > 10: health_score -= 10
                        if sodium > 600: health_score -= 20
                        elif sodium > 400: health_score -= 10
                        if fiber < 1: health_score -= 10
                        elif fiber >= 5: health_score += 10
                        
                        health_score = max(0, min(100, health_score))
                        
                        # Get image URL
                        image_url = product_data.get("image_url") or product_data.get("image_front_url") or ""
                        ...
                        products.append({
                            ...
                            "nutriments": {
                                "sugars_100g": sugar,
                                "sodium_100g": sodium / 1000,  # Back to grams
                                "fiber_100g": fiber
                            }
                        })
                else:
                ...
                print(f"❌ OpenFoodFacts API returned status {response.status_code}")
        
        except requests.Timeout:
            print(f"⚠️ OpenFoodFacts timeout for '{query}' after 5 seconds")
            # Return empty results on timeout instead of error
            products = []
        except Exception as e:
            print(f"⚠️ OpenFoodFacts error for '{query}': {e}")
            import traceback
            traceback.print_exc()
            products = []
        ...
        return {
            ...
            "total": 0,
            "query": query,
            "message": f"Search temporarily unavailable"
        }
