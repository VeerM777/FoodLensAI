...
class HealthScoreAgent:
    ...
    def analyze_health_impact(product_data: Dict[str, Any], user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        category = product_data.get("category", product_data.get("enhanced_category", "unknown"))
        
        print(f"📊 Analyzing health impact for: {product_name}")
        print(f"🏥 Health conditions: {health_conditions}")
        print(f"📋 Category: {category}")
        print(f"🧪 Ingredients available: {bool(ingredients_text)}")
        ...
        if product_type in ["carbonated_beverage", "beverage"] or "beverage" in product_type:
            health_result = health_analyzer.calculate_beverage_health_score(
                nutriments, "carbonated" if "carbonated" in product_type else "beverage", 
                health_conditions, ingredients_text, product_name
            )
            print(f"🥤 Using beverage-specific scoring for {product_type}")
        else:
            health_result = health_analyzer.calculate_health_score(
                nutriments, health_conditions, ingredients_text, product_name, category
            )
        
        # Extract score from result (now returns dict instead of int)
        health_score = health_result.get("score", 50) if isinstance(health_result, dict) else health_result
        quality_grade = health_result.get("quality_grade", "UNKNOWN") if isinstance(health_result, dict) else "UNKNOWN"
        ingredient_concerns = health_result.get("ingredient_concerns", []) if isinstance(health_result, dict) else []
        ...
        if ingredient_concerns:
            print(f"⚠️  Ingredient Concerns: {len(ingredient_concerns)} found")
        ...
        return {
            ...
            "analysis_summary": f"Health Score: {health_score}/100 - {verdict['title']} (Quality: {quality_grade})"
        }
